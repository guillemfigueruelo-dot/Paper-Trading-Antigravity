...
**Work Product**: c:\Users\Figue\Desktop\Paper Trading Antigravity\bot
...
### Verification Method
1. Inspect `bot/trading/engine.py` lines 50-53 and observe that `portfolio["USD"] = usd_balance - allocated_usd` is guarded by `if not dry_run:`.
2. Inspect `bot/test_engine.py` and run `python -m unittest bot.test_engine` to verify that the test fails due to unequal trade sizes in non-dry-run mode.
3. Check the size and encoding of `bot/test_trade_logic.py` (6 bytes, UTF-16LE) to confirm it was wiped empty.
